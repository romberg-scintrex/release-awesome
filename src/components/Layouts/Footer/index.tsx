...
export function Footer({ settings, profile }: Readonly<FooterProps>) {
  const year = new Date().getFullYear();

  // Pengaturan dynamic fallback jika profile disediakan
  const displayName = profile?.name ?? settings.name;
  const shortName = profile?.shortName ?? settings.shortName;
  const description = profile?.description || settings.description;
  const email = profile?.email || settings.email;

  const socialLinks = {
    github: profile?.social?.github || settings.social.github,
    linkedin: profile?.social?.linkedin || settings.social.linkedin,
    facebook: profile?.social?.facebook || settings.social.facebook,
    instagram: profile?.social?.instagram || settings.social.instagram,
  };

  const basePath = profile?.username ? `/${profile.username}` : "";
      ...
      <div className="container py-10 md:py-14">
        <div className="grid grid-cols-2 gap-x-6 gap-y-10 md:grid-cols-[1.5fr_1fr_1fr] md:gap-12">
          <div className="col-span-2 md:col-span-1">
            <Link href={basePath || "/"} className="inline-flex items-center font-display text-2xl font-bold tracking-tighter">
              <span className="text-ink-900 dark:text-white">{displayName}</span>
            </Link>
          ...
          <div>
            ...
            <ul className="mt-4 space-y-2 text-sm">
              ...
              ].filter(item => item.href).map(({ href, label, Icon }) => (
                <li key={label}>
                  <a
                    href={href}
        ...
        <div className="mt-10 flex flex-col items-center gap-2 border-t border-black/10 dark:border-white/10 pt-6 text-center sm:flex-row sm:items-center sm:justify-between sm:text-left md:mt-12">
          ...
          <p className="text-xs text-ink-400">
            Designed &amp; built by{" "}
            <span className="text-ink-700 dark:text-ink-200">{shortName}</span>{" "}
            with care &amp; coffee.
          </p>
        </div>
      </div>
